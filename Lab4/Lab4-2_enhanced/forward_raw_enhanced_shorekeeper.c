...
void init_forward_table() {
    ...
    //
    // 本配置用于: Shorekeeper (192.168.10.101) - 第二跳转发器
    // 数据流: Camellya → Phrolova → Shorekeeper → Carlotta → Jinhsi

    // 条目1: 目标Jinhsi，下一跳是Carlotta
    strcpy(forward_table[0].dest_ip, "192.168.10.102");  // 最终目标IP (Jinhsi)
    forward_table[0].dest_mac[0] = 0x00;  // Carlotta的MAC
    forward_table[0].dest_mac[1] = 0x50;
    forward_table[0].dest_mac[2] = 0x56;
    forward_table[0].dest_mac[3] = 0x3e;
    forward_table[0].dest_mac[4] = 0x9d;
    forward_table[0].dest_mac[5] = 0x96;
    strcpy(forward_table[0].description, "Next->Carlotta");
